...
class FuRegnrScrapySpider(scrapy.Spider):
    ...
    def get_one_by_one(self, line):
        __RequestVerificationToken = self.get__RequestVerificationToken()
        recaptchaClientToken = self.resolve_captcha(line=line)
        self.get_details(line, recaptchaClientToken, __RequestVerificationToken)

    def resolve_captcha(self, line):
        Registreringsnummer = line[18]
    ...
    def get__RequestVerificationToken(self):
        sess = requests.Session()
        headers = {
            'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.97 Safari/537.36',
            'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3'
    ...
    def get_details(self, line, recaptchaClientToken, __RequestVerificationToken):
        sess = requests.Session()
        Registreringsnummer = line[18]
        headers = {
            'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.97 Safari/537.36',
